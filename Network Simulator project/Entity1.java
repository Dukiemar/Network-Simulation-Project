...
{    
    ...
    public Entity1()
    {
        ...
        for (int i = 0; i < NetworkSimulator.NUMENTITIES; i++)
    	{
    		...
    		minCost[i] = infinity;
    		System.out.println("Initial Mincost Entity1= "+minCost[i]);
    	}
        System.out.println();
    ...
    public void update(Packet p)
    {
        ...
        if((p_dest==1) && isNeighbour(p_src)==true)//account for the issue in the simulator were packets are sent to non neighbours.
        {
    		    ...
    		    if(distanceTable[x][p_src] > p.getMincost(x) + minCost[p_src])
    		    {
    			   ...
    			   if (p.getMincost(x) + minCost[p_src] < minCost[x])
    			   {
    				     	   ...
    				     	   for (int i = 0; i < 4; i++)
    			     {
                		...
                		System.out.println("Updated Mincost Entity1= "+minCost[i]);
                     }
